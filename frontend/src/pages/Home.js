...
import { TrendingUp, Users, Filter, Search, Play, Eye, Heart, MoreVertical, } from "lucide-react";
...
export default function Home() {
    ...
    const ProjectCard = ({ project }) => {
        const progressPercentage = project.targetAmount
            ...
            : 0;
        return (<motion.div layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} whileHover={{ scale: 1.02 }} className="group">
        <Card className="h-full overflow-hidden hover:shadow-lg transition-all duration-200">
          ...
          <div className="relative aspect-video bg-muted overflow-hidden">
            {project.thumbnailUrl ? (<img src={project.thumbnailUrl} alt={project.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-200"/>) : (<div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-muted to-muted-foreground/10">
            ...
            {/* Status Badge */}
            <Badge variant={project.status === 'active' ? 'default' : 'secondary'} className="absolute top-3 right-3">
          ...
          <CardContent className="p-4">
            ...
            <div className="space-y-2 mb-4">
              ...
              <div className="w-full bg-muted rounded-full h-2">
                <div className="bg-primary h-2 rounded-full transition-all duration-300" style={{ width: `${Math.min(progressPercentage, 100)}%` }}/>
              </div>
            ...
            <div className="flex items-center justify-between">
              ...
              {project.mlScore && (<Badge variant="outline" className="text-xs">
    ...
    const ProjectCardSkeleton = () => (<Card className="h-full overflow-hidden">
    ...
    return (<div className="container-xl py-8">
      ...
      <div className="mb-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            {liveUpdates.length > 0 && (<Badge variant="destructive" className="animate-pulse">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div className="relative">
              ...
              <Input placeholder="Rechercher un projet..." value={filters.search} onChange={(e) => updateFilter('search', e.target.value)} className="pl-10"/>
            </div>
      ...
      {!isLoading && (<div className="flex items-center justify-between mb-6">
          ...
          {pagination && pagination.totalPages > 1 && (<p className="text-sm text-muted-foreground">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {isLoading ? (
        // Loading skeletons
        Array.from({ length: 8 }).map((_, index) => (<ProjectCardSkeleton key={index}/>))) : error ? (
        // Error state
      ...
      {/* Load More */}
      {pagination && pagination.page < pagination.totalPages && (<div className="flex justify-center mt-8">
