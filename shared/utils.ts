...
import { 
  PROJECT_CATEGORIES, 
  PROFILE_CAUTION_MINIMUMS, 
  INVESTMENT_STATUS, 
  INVESTMENT_VOTES_MAPPING,
  VISUPOINTS_CONFIG,
  DEFAULT_CATEGORY_SCORE, 
  DEFAULT_COLOR_CLASS, 
  DEFAULT_CAUTION_MINIMUM,
  REPORT_TYPES
} from './constants';
 ...
 */
export function calculateVotes(amount: number): number {
  const closestAmount = Object.keys(INVESTMENT_VOTES_MAPPING)
    .map(Number)
    .find(amt => amt >= amount) || 20;
  
  return INVESTMENT_VOTES_MAPPING[closestAmount as keyof typeof INVESTMENT_VOTES_MAPPING] || 10;
}
 ...
 */
export function eurToVisuPoints(eur: number): number {
 ...
 * @returns EUR amount
 */
export function visuPointsToEur(points: number): number {
 ...
 */
export function isVisuPointsWithdrawable(points: number): boolean {
 ...
 */
export function calculateROI(currentValue: number, initialAmount: number): number {
 ...
 */
export function getReportTypeInfo(reportType: string): { label: string; description: string } {
  return REPORT_TYPES[reportType as keyof typeof REPORT_TYPES] || {
    label: 'Autre',
